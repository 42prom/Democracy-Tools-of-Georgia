...
class _ConfirmVoteScreenState extends State<ConfirmVoteScreen> {
  ...
  Future<void> _submitVote() async {
    ...
    try {
      ...
      // Save activity record with backend-confirmed reward info + signed receipt
      await _storageService.saveActivityItem(
        ActivityItem(
          ...
          votedOptionId: widget.selectedOption.id,
          votedOptionText: widget.selectedOption.text,
          receipt: response['receipt'], // Save the signed Ed25519 receipt
        ),
      );
        ...
        Navigator.of(context).pushReplacement(
          MaterialPageRoute(
            builder: (context) => VoteReceiptScreen(
              ...
              merkleRoot: response['merkleRoot'] as String?,
            ),
          ),
        );
      }
