...
void main() {
      ...
      final allFiles = await libDir
          ...
          .where(
            (entity) =>
                ...
                !entity.path.contains(RegExp(r'models[/\\]')), // Ignore models
          )
          .toList();
  ...
  group('Citizen Features Only', () {
    ...
    test('Bottom navigation has only citizen tabs', () async {
      ...
      // Expected tabs: Voting, Wallet, Settings (via localization keys)
      expect(content.contains('nav_voting'), isTrue);
      expect(content.contains('nav_wallet'), isTrue);

      // No admin tab
      expect(content.toLowerCase().contains('admin'), isFalse);
      expect(
        content.contains('nav_settings'),
        isTrue,
