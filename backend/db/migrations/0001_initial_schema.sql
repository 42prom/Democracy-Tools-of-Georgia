...
CREATE TABLE IF NOT EXISTS public.nonces (
    nonce character varying(64) NOT NULL PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS public.votes (
    ...
    vote_hash character(64),
    previous_hash character(64),
    chain_hash character(64)
);

CREATE TABLE IF NOT EXISTS public.vote_anchors (
    id uuid DEFAULT gen_random_uuid() NOT NULL PRIMARY KEY,
    poll_id uuid NOT NULL REFERENCES public.polls(id),
    start_vote_id uuid REFERENCES public.votes(id),
    end_vote_id uuid REFERENCES public.votes(id),
...
-- votes table moved up to fix dependency in vote_anchors
...
-- H3: Per-poll symmetric encryption keys (optional, off by default)
...
CREATE INDEX IF NOT EXISTS idx_vote_anchors_poll ON public.vote_anchors(poll_id);
CREATE INDEX IF NOT EXISTS idx_vote_anchors_status ON public.vote_anchors(status);
...
CREATE INDEX IF NOT EXISTS idx_votes_demographics ON public.votes USING GIN (demographics_snapshot);
CREATE INDEX IF NOT EXISTS idx_votes_poll_chain ON public.votes(poll_id, created_at DESC);
