...
export async function getPollById(pollId: string): Promise<{ poll: Poll; options: PollOption[] } | null> {
  ...
  let referendum_question;
  let referendum_threshold;
  let election_question;

  if (poll.type === 'referendum' && poll.audience_rules) {
    referendum_question = (poll.audience_rules as any).referendum_question || '';
    referendum_threshold = (poll.audience_rules as any).referendum_threshold || 50;
  } else if (poll.type === 'election' && poll.audience_rules) {
    election_question = (poll.audience_rules as any).election_question || '';
  }
