...
export class AuditExportService {
  ...
  async generateExport(options: AuditExportOptions): Promise<{
    ...
    if (options.includeVoteLevelData) {
      csvContent += createCsvRow(['=== INDIVIDUAL VOTE RECORDS ===', '', '', '', '', '']);
      csvContent += createCsvRow([
        'Vote ID',
        'Option ID',
        'Option Text',
      ...
      async read() {
        try {
          if (!headersSent) {
            ...
            if (options.includeVoteLevelData) {
              header += createCsvRow(['=== INDIVIDUAL VOTE RECORDS ===', '', '', '', '', '', '']);
              header += createCsvRow(['Vote ID', 'Option ID', 'Option Text', 'Age Bucket', 'Gender', 'Region', 'Timestamp']);
