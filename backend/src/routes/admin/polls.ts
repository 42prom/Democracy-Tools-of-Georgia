  ...
  try {
    ...
    const pollsWithOptions = await Promise.all(
      pollsResult.rows.map(async (poll) => {
        ...
        // Extract referendum/election metadata from audience_rules
        if (poll.type === 'referendum' && poll.audience_rules) {
          result.referendum_question = (poll.audience_rules as any).referendum_question || '';
          result.referendum_threshold = (poll.audience_rules as any).referendum_threshold || 50;
        } else if (poll.type === 'election' && poll.audience_rules) {
          result.election_question = (poll.audience_rules as any).election_question || '';
        }
...
router.get('/:id', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    // Extract referendum/election metadata from audience_rules for applicable polls
    if (poll.type === 'referendum' && poll.audience_rules) {
      response.referendum_question = (poll.audience_rules as any).referendum_question || '';
      response.referendum_threshold = (poll.audience_rules as any).referendum_threshold || 50;
    } else if (poll.type === 'election' && poll.audience_rules) {
      response.election_question = (poll.audience_rules as any).election_question || '';
    }
...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      // For referendum/election type, store metadata in audience_rules
      ...
      } else if (pollData.type === 'election') {
        pollData.audience_rules = {
          ...pollData.audience_rules,
          election_question: pollData.election_question || '',
        };
      }
...
router.get(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      let breakdowns;
      const { breakdownBy } = req.query;
      console.log(`[Admin Poll Results] PollId=${id}, breakdownBy=`, breakdownBy);
      
      if (breakdownBy) {
        let dimensions: string[] = [];
        if (Array.isArray(breakdownBy)) {
          dimensions = breakdownBy as string[];
        } else if (typeof breakdownBy === 'string') {
          dimensions = breakdownBy.split(',').filter(Boolean);
        }
          
        if (dimensions.length > 0) {
          breakdowns = await getPollDemographics(String(id), dimensions);
        }
      }
      ...
      res.json({
        ...
        breakdowns,
        metadata: {
          kThreshold: 0, // Admin sees raw counts for now (except demographics which are k-anonymized)
          suppressedCells: 0,
          lastUpdated: new Date().toISOString()
        }
      });
