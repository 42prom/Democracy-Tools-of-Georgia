 ...
 * =================
 * 
 * Handles the multi-step user enrollment and biometric verification process.
 * 
 * Flows:
 * 1. NFC Reading (Optional/Mandatory depending on config)
 * 2. Document Scanning (Optional/Mandatory)
 * 3. Liveness & Face Match (Critical Security Step)
 * 4. Credential Issuance (Blockchain Integration)
 * 
 * Security:
 * - Rate Limiting (IP + PN Hash)
 * - Session Management (EnrollmentSession)
...
router.post('/verify-biometrics', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const credentialToken = issueCredentialForSubject(userId, demographics, 24 * 60 * 60); // 24 hours strict
