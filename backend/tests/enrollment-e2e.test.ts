...
// No longer using live URL logic for internal tests
...
describe('Enrollment E2E Flow', () => {
  ...
  beforeAll(async () => {
    // Connect to Redis for rate limiting and health checks
    const { connectRedis } = await import('../src/db/redis');
    await connectRedis();
    ...
    // Ensure at least one region exists for Step 2 and Step 5
    await pool.query(`
      INSERT INTO regions (code, name_en, name_ka, active)
      VALUES ($1, $2, $3, true)
      ON CONFLICT (code) DO UPDATE SET active = true
    `, ['reg_tbilisi', 'Tbilisi', 'თბილისი']);
  ...
  describe('Health Check', () => {
    it('should confirm API is running', async () => {
      ...
      // In test environment, "degraded" is acceptable if biometric service is down
  ...
  describe('Step 1: Fetch Verification Policy', () => {
    it('should return verification settings', async () => {
      ...
      const res = await request(app)
        .get(BASE_URL + '/settings/verification')
        .expect(200);
  ...
  describe('Fetch Regions', () => {
    it('should return list of regions', async () => {
      ...
      const res = await request(app)
        .get(BASE_URL + '/enrollment/regions')
        .expect(200);
  ...
  describe('Check User Status (should not exist)', () => {
    it('should report user does not exist', async () => {
      console.log('Checking user status...');

      const res = await request(app)
        .post(BASE_URL + '/enrollment/status')
  ...
  describe('Submit NFC Data (Start Enrollment)', () => {
    it('should create enrollment session and return nonce', async () => {
      console.log('Starting NFC enrollment...');
      ...
      const docRes = await request(app)
        .post(BASE_URL + '/enrollment/document')
        .send(docPayload)
        .expect(200);
  ...
  describe('Update Profile (Region Selection)', () => {
    it('should update profile with region', async () => {
      ...
      const res = await request(app)
        .post(BASE_URL + '/enrollment/profile')
        .send(profilePayload)
        .expect(200);
  ...
  describe('Verify Biometrics (Liveness + Face Match)', () => {
    it('should fail WITHOUT nonce (testing nonce validation)', async () => {
      console.log('Testing nonce validation (should fail)...');
      ...
      const res = await request(app)
        .post(BASE_URL + '/enrollment/verify-biometrics')
    ...
    it('should succeed WITH correct nonce', async () => {
      console.log('Verifying biometrics with nonce...');
      ...
      const nfcRes = await request(app)
        .post(BASE_URL + '/enrollment/nfc')
        .send(nfcPayload)
        .expect(200);
  ...
  describe('Verify Session (Login Flow)', () => {
